...
//    }
    ...
    public List<Issue> SearchByStatusAndAssignedIssues(Integer assigned_to_id, Integer status_id) {
        try {
            Params params = new Params()
    ...
    public List<Issue> SearchByPriorityAndAssignedIssues(Integer assigned_to_id, Integer priority_id) {
        try {
            Params params = new Params()
                    .add("assigned_to_id", Integer.toString(assigned_to_id))
                    .add("priority_id", Integer.toString(priority_id));
    ...
    public List<Issue> SearchByLessHalfDoneRatioAndAssignedIssues(Integer assigned_to_id) {
        try {
            Params params = new Params()
                    .add("assigned_to_id", Integer.toString(assigned_to_id))
                    .add("done_ratio", ">=" + 0)
    ...
    public List<Issue> SearchByMoreHalfDoneRatioAndAssignedIssues(Integer assigned_to_id) {
        try {
            Params params = new Params()
                    .add("assigned_to_id", Integer.toString(assigned_to_id))
                    .add("done_ratio", ">=" + 50)
    ...
    public Map<String, Float> SearchLess40HourDWeeks(Integer assigned_to_id, Integer year, Integer month) {
        try {
            Map<String, Float> weeks = new HashMap<>();
            ...
            while (calendar.get(Calendar.MONTH) < month) {
                ...
                if (hours < weekWorkHours)
                    weeks.put(weekString + " - " + dayString, hours);
            }
            return weeks;
    ...
    public List<TimeEntry> WeekReportByUsers(Integer project_id) {
        try {
            Map<String, String> params = new HashMap<>();
            params.put("set_filter", "1");
            params.put("project_id", Integer.toString(project_id));
            params.put("sort", "spent_on:desc");
            params.put("op[spent_on]", "*");
            params.put("f[]", "");
            params.put("group_by", "");
            params.put("t[]", "hours");
            params.put("columns", "week");
            params.put("criteria[]", "user");
            return mgr.getTimeEntryManager().getTimeEntries(params).getResults();
        } catch (Exception e) {
            e.printStackTrace();
        }
